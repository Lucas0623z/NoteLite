...
│
├── app
│   └── build.gradle // Building NoteLite application
│
├── docs
│   └── pdf
...
└── packaging
 ...
    └── dev
    └── omr.properties // Mapping the .omr extension to NoteLite application
</pre>

## Building an installer

Each created installer includes a (shrunk) Java Runtime Environment (JRE), the NoteLite application and all the
...
2. `collectJars`: task to collect the `notelite.jar` just built, with all the jars it depends on
...
The draft is now present in the [Releases section](https://github.com/ORG/notelite/releases)
of the NoteLite repository, but since it is a draft, it is visible only by the repository authors.
...
a new Latest Release is defined in NoteLite repository.
...
2. Retrieve the descriptor of NoteLite latest release
3. Retrieve the download URL of the (first) Windows installer among the release assets
4. Use `wingetcreate.exe` to update and submit the winget manifest for NoteLite
...
The purpose of this action is to build a Scoop manifest that references the NoteLite latest release.
The sole artifact of this action can be downloaded at the provided URL:

![](./assets/scoop-artifact-download.png)

The downloaded archive `scoop-manifest.zip` contains the file `notelite.json` which is the Scoop manifest.

Here is the example of the `notelite.json` for the 5.10.0 release:
```json
{
 "version": "5.10.0",
 "description": "Optical Music Recognition",
 "homepage": "https://github.com/ORG/notelite",
 "license": "AGPL-3.0-or-later",
 "architecture": {
 "64bit": {
 "url": "https://github.com/ORG/notelite/releases/download/5.10.0/NoteLite-5.10.0-windowsConsole-x86_64.msi",
 "hash": "sha256:74857d40a41dd5ce5b2f419dc1c6a844f58e542a2b2454e658956ad1aca33953"
 }
 },
 "extract_dir": "NoteLite",
 "bin": "NoteLite.exe",
 "shortcuts": [
 [
 "NoteLite.exe",
 ...
 "checkver": {
 "github": "https://github.com/ORG/notelite"
 },
 "autoupdate": {
 "architecture": {
 "64bit": {
 "url": "https://github.com/ORG/notelite/releases/download/$version/NoteLite-$version-windowsConsole-x86_64.msi"
 }
 }
 }
}
...
2. Add the `notelite.json` file into the `bucket` folder
...
To save the token value on NoteLite site, [https://github.com/ORG/notelite](https://github.com/ORG/notelite):
- Repository Settings
